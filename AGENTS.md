...
- 本 skill 安装在用户级 skill 根（`$DSH_HOME/skills`，DSH_HOME 缺省为主目录下的
  `.dsh`；Windows 上目录名为 `Skills`，大小写不敏感）。**绝对路径不写死**——需要时
  由本文件所在位置直接推导，skill 整体迁移后相对关系不变。
- 宿主按目录式发现：本目录下的 `SKILL.md` 即入口，frontmatter 的 `name` 必须与目录名
  逐字相同。

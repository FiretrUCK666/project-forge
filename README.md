...
**这个 skill 的脚本一行 Python 都不用。** 它对 Python 的全部认知只是「识别出这是个
Python 项目，并给出该项目的构建命令」（例如 `python -m pytest`）——那是**读取**项目
信息，不是运行 Python。

唯一沾到 Python 的地方是一个**可选**步骤：`references/docs-set.md` 建议用 `oil-tone`
这个独立 skill 校对面向人的文稿，而它的检查脚本需要 Python。**它不可用时直接跳过，
并在汇报里写明「未做文风校对」**——不装 Python 完全不影响本 skill 的任何功能。

...
**打包产物必须进版本库**：发布用的是打包出来的存档（`.vsix`），而安装方不会替你构建。
如果仓库里只有源码、构建产物是发布时现做的，那 CI 里必须有一条「重建后与提交一致」的
检查，否则产物与源码迟早脱节。
...
- **包含规则**：打包工具按一个忽略文件决定排除哪些内容（具体文件名与语法见官方文档的
  「Publishing Extensions」一节）。默认会排除一批常见目录，但**不要依赖默认**——
